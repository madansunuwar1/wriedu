...
<div class="card">
  ...
  <div class="card-body">
    ...
    <form id="financeForm" action="{{ route('backend.comission.store') }}" method="POST" class="needs-validation" novalidate>
      @csrf
    ...
    <select class="form-control" id="university" name="university" required>
        ...
        @foreach($data_entries as $data_entry)
            <option value="{{ $data_entry->newUniversity }}">{{ $data_entry->newUniversity }}</option>
        @endforeach
    </select>
    <div class="valid-feedback">Looks good!</div>
    <div class="invalid-feedback">Please select a university name.</div>
</div>
        ...
        <div class="col-md-6 mb-3">
          ...
          <select class="form-control" id="partner" name="partner" required>
            ...
            <option value="{{ $data_entrie->id }}">{{ $data_entrie->intake }}</option>
            @endforeach
          </select>
      ...
      <!-- Intensive Commission section -->
      <div class="col-md-6 mb-3">
        <label class="form-label">Intensive Commission?</label>
        <div class="form-check">
          <input class="form-check-input" type="radio" name="has_intensive_commission" id="intensiveCommissionYes" value="yes" {{ old('has_intensive_commission') == 'yes' ? 'checked' : '' }}>
          <label class="form-check-label" for="intensiveCommissionYes">
            Yes
          </label>
        </div>
        <div class="form-check">
          <input class="form-check-input" type="radio" name="has_intensive_commission" id="intensiveCommissionNo" value="no" {{ old('has_intensive_commission', 'no') == 'no' ? 'checked' : '' }}>
          <label class="form-check-label" for="intensiveCommissionNo">
            No
          </label>
        </div>
      </div>

      <div class="col-md-6 mb-3" id="intensiveCommissionFields" style="display: none;">
        <label class="form-label" for="intensive_commission">Intensive Commission</label>
        <input type="text" class="form-control" id="intensive_commission" name="intensive_commission"
               placeholder="Enter Intensive Commission" value="{{ old('intensive_commission') }}">
        <div class="invalid-feedback">Please provide intensive commission.</div>
      </div>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Intensive Commission
    const intensiveYesRadio = document.getElementById('intensiveCommissionYes');
    const intensiveNoRadio = document.getElementById('intensiveCommissionNo');
    const intensiveFields = document.getElementById('intensiveCommissionFields');

    function toggleIntensiveFields() {
      intensiveFields.style.display = intensiveYesRadio.checked ? 'block' : 'none';
    }

    // Initial state
    toggleIntensiveFields();

    // Event listeners
    intensiveYesRadio.addEventListener('change', toggleIntensiveFields);
    intensiveNoRadio.addEventListener('change', toggleIntensiveFields);
  });
</script>
@endsection