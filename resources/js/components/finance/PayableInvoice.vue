  ...
  <div class="main-container">
    <div class="widget-content searchable-container list">
      ...
      <div v-if="isLoading" class="loading-container text-center p-5">
        ...
        <p class="mt-2 text-muted">Loading Payable Accounts...</p>
      </div>
      ...
      <div v-else>
        ...
        <div v-if="!processedApplications.length" class="no-data-container text-center p-5 mx-auto bg-light rounded" style="max-width: 800px;">
          ...
          <h4 class="text-muted">No Payable Accounts Found</h4>
          <p class="text-secondary">There are no applications matching the criteria for accounts payable.</p>
          <div class="mt-3">
            <button @click="retryLoad" class="btn btn-outline-primary btn-sm me-2">
              <i class="fas fa-refresh me-1"></i>Refresh Data
            </button>
            <button @click="debugMode = true" class="btn btn-outline-info btn-sm">
              <i class="fas fa-bug me-1"></i>Show Debug Info
            </button>
          </div>
          
          <!-- Raw Data Debug -->
          <div v-if="debugMode" class="mt-4 text-start">
            <div class="card">
              <div class="card-header">
                <h6>Raw Data Debug</h6>
              </div>
              <div class="card-body">
                <pre class="small">{{ JSON.stringify({ rawApplications, commissions }, null, 2) }}</pre>
              </div>
            </div>
          </div>
        </div>
          ...
          >
            ...
            <div class="totals-section">
              <div class="totals-box">
                ...
                <div class="total-row editable-field" v-if="type === 'payable'" @dblclick="startEditing(app.id, 'exchange_rate', app.exchangeRate)">
                  <span>Exchange Rate</span>
                  <div class="exchange-rate-controls">
                    <span v-if="!isEditing(app.id, 'exchange_rate')" class="display-value">
                      NPR {{ formatCurrency(app.exchangeRate, 1) }}
                    </span>
                ...
                <div class="total-row paid-row editable-field" v-if="type === 'payable'" @dblclick="startEditing(app.id, 'paid', app.paidValue)">
                  <span>Paid</span>
...
export default {
  ...
  computed: {
    processedApplications() {
      // Add more defensive checks and logging
      console.log('Computing processedApplications...');
      console.log('rawApplications:', this.rawApplications);
      console.log('commissions:', this.commissions);

      if (!this.rawApplications || !this.commissions) {
        console.warn('Data not loaded yet - rawApplications or commissions is null/undefined');
        return [];
      }

      if (!Array.isArray(this.rawApplications) || !Array.isArray(this.commissions)) {
        console.warn('Invalid data structure for processing applications');
        console.log('rawApplications type:', typeof this.rawApplications);
        console.log('commissions type:', typeof this.commissions);
        return [];
      }

      if (this.rawApplications.length === 0) {
        console.log('No raw applications to process');
        return [];
      }

      if (this.commissions.length === 0) {
        console.log('No commissions data available, processing with default values');
        // Continue processing even without commissions
      }

      const processed = this.rawApplications
        .filter(app => {
          if (!app || !app.id) {
        ...
        .map(app => {
          try {
            const result = this.processApplication(app);
            console.log('Processed application:', app.id, result);
            return result;
          } catch (error) {
      ...
      console.log('Final processed applications:', processed);
      return processed;
    }
  },
  methods: {
    processApplication(app) {
      ...
      // Only try to find commission if we have commissions data
      if (this.commissions && this.commissions.length > 0 && app.university && app.english) {
      ...
      let paidValue = 0;
      let exchangeRate = 135; // Default exchange rate

      // Get commission transaction data
      const commissionTransaction = app.commission_transaction || {};
      
      // Debug logging
      console.log('Commission Transaction for app', app.id, ':', commissionTransaction);
      
      // Check if there's commission payable data
      if (commissionTransaction && Object.keys(commissionTransaction).length > 0) {
        // Check if the transaction type matches our current type
        if (commissionTransaction.type === this.type) {
          // Use the actual values from database
          if (commissionTransaction.paid !== null && commissionTransaction.paid !== undefined) {
            paidValue = parseFloat(commissionTransaction.paid) || 0;
          }
          
          if (commissionTransaction.exchange_rate !== null && commissionTransaction.exchange_rate !== undefined) {
            exchangeRate = parseFloat(commissionTransaction.exchange_rate) || 135;
          }
          
          console.log(`Using database values - Paid: ${paidValue}, Exchange Rate: ${exchangeRate}`);
        } else {
          console.log(`Type mismatch - Transaction type: ${commissionTransaction.type}, Current type: ${this.type}`);
        }
      } else {
        console.log('No commission transaction data found, using defaults');
      }
    ...
    async fetchData(id) {
      ...
      try {
        console.log('Making API request to:', `/api/payable/${encodeURIComponent(id)}`);
        const response = await axios.get(`/api/payable/${encodeURIComponent(id)}`);
        ...
        // Extract basic data with more defensive checks
        this.rawApplications = Array.isArray(payload.applications) ? payload.applications : [];
        this.commissions = Array.isArray(payload.commissions) ? payload.commissions : [];
        
        console.log('Set rawApplications:', this.rawApplications);
        console.log('Set commissions:', this.commissions);
        
        // Map commission transactions to applications
        const commissionTransactions = Array.isArray(payload.commissionTransactions) ? payload.commissionTransactions : [];
        
        // Enhance applications with their commission transactions
        this.rawApplications = this.rawApplications.map(app => {
          // Find matching commission transaction for this application
          const matchingTransaction = commissionTransactions.find(
            trans => trans.application_id === app.id && trans.type === this.type
          );
          
          return {
            ...app,
            commission_transaction: matchingTransaction || null
          };
        });

        this.retryAttempts = 0;

        console.log(`Final data loaded - ${this.rawApplications.length} applications and ${this.commissions.length} commissions`);
        console.log('Sample application:', this.rawApplications[0]);
        ...
        this.isLoading = false;
        console.log('fetchData completed. isLoading:', this.isLoading);
      }
    },
    ...
    async saveData(app, field, value) {
      ...
      try {
        const payable = {
          ...
          value: value,
          type: this.type,
          commissionpayable_id: app.commission_transaction?.commissionpayable_id
        };

        console.log('Saving data:', payable);

        const response = await axios.post(`/api/commission-payables/${app.id}`, payable);
    ...
    async downloadPDF() {
      ...
      try {
        ...
        const fileName = `WRI-Accounts-Payable-${new Date().toISOString().slice(0, 10)}.pdf`;
        pdf.save(fileName);
