...
<form action="{{ route('backend.leadcomment.update', $lead_comments->id) }}" method="POST">

    @csrf
    @method('PUT')
        <h1>Comment</h1>
        <label for="comment">Leave a Comment</label>
        <textarea id="comment" name="comment" placeholder="Type your comment here...">{{ old('comment', $lead_comments->comment) }}</textarea>
